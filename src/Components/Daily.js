...
class Daily extends Component {
    ...
    componentDidMount() {
        let url = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + this.props.ticker + "&outputsize=full&apikey=X3775KOZSSR1IJ79";
        console.log(url)
        axios.get(url)
            .then(res => {
                console.log(url)
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        axios.get("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + this.props.ticker + "&outputsize=full&apikey=X3775KOZSSR1IJ79")
            .then(res => {
                let data = res.data['Time Series (Daily)']
