...
class App extends Component {
    ...
    financialDashboard = (event) => {
        ...
        axios.get("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + this.state.ticker + "&outputsize=full&apikey=X3775KOZSSR1IJ79")
            .then(res => {
        ...
        axios.get("https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=" + this.state.ticker + "&apikey=X3775KOZSSR1IJ79")
            .then(res => {
                let keys = Object.keys(res.data);
                if (keys[0] === "Error Message" || keys[0] == "Note") {
                    this.setState({error: true})
        ...
        axios.get("https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=" + this.state.ticker + "&interval=5min&apikey=X3775KOZSSR1IJ79")
            .then(res => {
                let keys = Object.keys(res.data);
                if (keys[0] === "Error Message" || keys[0] === "Note") {
                    this.setState({error: true})
